...
#     - The .md file will be `YYYY-MM-DD-[url_slug].md` and the permalink will be `https://[yourdomain]/posters/YYYY-MM-DD-[url_slug]`
...
for row, item in posters.iterrows():
    ...
    if len(str(item.poster_url)) > 3:
        md += "\n[More information here](" + item.poster_url + ")\n"
...
# These files are in the posters directory, one directory below where we're working from.
